...
export async function runCronTasks(env: any): Promise<{ ran: number; skipped: number; errors: number }> {
  ...
  for (const item of items) {
    ...
    try {
      const status = await runTask(env, task, 'cron');
...
async function runTask(env: any, task: CronTask, source: OutboundSource = 'default'): Promise<{ ok: boolean; error?: string }> {
  try {
...
const cronPlugin: BackendPlugin = {
  manifest,
  router,
};
